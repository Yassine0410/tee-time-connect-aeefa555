...
function computeRankings(rounds: ReturnType<typeof useRounds>['data']): PlayerRanking[] {
  if (!rounds) return [];
  ...
  return Array.from(statsMap.values()).sort((a, b) => b.wins - a.wins || a.handicap - b.handicap);
}
...
export default function Leaderboard() {
  ...
  const { t } = useLanguage();
  const rankings = computeRankings(rounds);
    ...
    return (
      <div className="screen-content">
        ...
        <div className="text-center py-12">
          ...
          <p className="text-foreground font-medium">{t('leaderboard.noRankings')}</p>
          <p className="text-sm text-muted-foreground mt-1">{t('leaderboard.noRankingsHint')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="screen-content">
            ...
            return (
              <div
                ...
                className={`flex flex-col items-center animate-fade-in ${
                  isFirst ? 'order-2' : rankIndex === 1 ? 'order-1' : 'order-3'
                }`}
              >
                ...
                <span className="golf-badge-primary mt-1 text-[10px]">
                  {entry.wins} {entry.wins > 1 ? t('leaderboard.victories') : t('leaderboard.victory')}
                </span>
                <div className={`w-20 rounded-t-xl mt-2 bg-gradient-golf opacity-90 ${isFirst ? 'h-24' : rankIndex === 1 ? 'h-16' : 'h-12'}`} />
              </div>
            );
          })}
        </div>
      ...
      <div className="golf-card p-0 overflow-hidden">
        <div className="px-4 py-3 border-b border-border">
          <h3 className="font-semibold text-foreground text-sm">{t('leaderboard.allPlayers')}</h3>
        </div>
        {rankings.map((entry, index) => (
          ...
          >
            <span className={`w-7 text-center font-bold text-sm ${index < 3 ? 'text-primary' : 'text-muted-foreground'}`}>
              {index + 1}
            </span>
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium text-foreground text-sm truncate">
                {entry.name}
                {profile && entry.id === profile.id && <span className="text-xs text-muted-foreground ml-1">{t('leaderboard.you')}</span>}
              </p>
            ...
            <div className="text-right">
              ...
              <p className="text-[10px] text-muted-foreground">{t('leaderboard.played', { count: entry.played })}</p>
            </div>
          </div>
        ))}
      </div>
